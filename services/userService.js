...
const createUser = async (email, address, password, phone_number, policy_agreed, username ) => {
  ...
  if (!(email && password && username)) { 
    console.log(err)
    console.log('서비스단 왈, 필수정보미입력')
    // res.status(400).json({message: '필수 항목을 입력해주세요'}) //res는 controller에서
  } 
  ...
  const hashedPassword = await bcrypt.hash(password, salt);
    ...
    await userModel.createUser(email, address, hashedPassword, phone_number, policy_agreed, username );
